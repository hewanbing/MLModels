...
# for 18 or 34 layers
class ResNet2DBlock_v1(keras.Model):

    def __init__(self, filters, block_name, downsample=False, stride=1):
        self.expasion = 1
        super(ResNet2DBlock_v1, self).__init__()
        ...
        self.bn2a = keras.layers.BatchNormalization(axis=3)
        ...
        self.bn2b = keras.layers.BatchNormalization(axis=3)
        ...
        if self.downsample:
            ...
            self.bn_shortcut = keras.layers.BatchNormalization(axis=3)
...
class ResNet2DBlock_v2(keras.Model):

    def __init__(self, filters, block_name, downsample=False, stride=1, **kwargs):
        self.expasion = 4
        super(ResNet2DBlock_v2, self).__init__(**kwargs)
...
class ResNet2D(keras.Model):
    def __init__(self, block, layers, num_classes=1, **kwargs):
        super(ResNet2D, self).__init__(**kwargs)

        self.padding = keras.layers.ZeroPadding2D((1, 1))
...
def resnet18(classes=1):
    return ResNet2D(ResNet2DBlock_v1, [2, 2, 2, 2], num_classes=classes)


def resnet38(classes=1):
    return ResNet2D(ResNet2DBlock_v1, [3, 4, 6, 3], num_classes=classes)


def resnet50(classes=1):
    return ResNet2D(ResNet2DBlock_v2, [3, 4, 6, 3], num_classes=classes)


def resnet101(classes=1):
    return ResNet2D(ResNet2DBlock_v2, [3, 4, 23, 3], num_classes=classes)


def resnet152(classes=1):
    return ResNet2D(ResNet2DBlock_v2, [3, 8, 36, 3], num_classes=classes)
